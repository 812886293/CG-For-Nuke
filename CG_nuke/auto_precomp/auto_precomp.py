...
def createArnoldPrecomp():
    ...
    unpremult = nuke.nodes.Unpremult(channels="all", name="Unpremult_all")
    unpremult.setXYpos(read_x, read_y + 200)
    unpremult.setInput(0, sel)

    # 创建Read节点下方的第一个Dot节点（Dot74）
    # 这个Dot是整个网络的起点，连接Unpremult节点
    read_dot = nuke.nodes.Dot(note_font_size=35)
    read_dot.setXYpos(read_x + 34, dot_y)  # X偏移34像素
    read_dot.setInput(0, unpremult)  # 连接到Unpremult节点
    ...
    if len(light_group_outputs) > 1:
        ...
        if has_volume and volume_x is not None:
            ...
            copy_output = copy_node
            premult_x = volume_x

        premult = nuke.nodes.Premult(channels="all", name="Premult_all")
        premult.setXYpos(premult_x, final_merge_y + 250)
        premult.setInput(0, copy_output)
